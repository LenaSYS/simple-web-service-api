...
<?PHP
			...
			if($button=='Save'){
					...
					if($size==""){
							$stmt->bindValue(':SIZE', null, PDO::PARAM_INT);
					}else{
							$stmt->bindParam(':SIZE',$size);
					}
					if($cost==""){
							$stmt->bindValue(':COST', null, PDO::PARAM_INT);
					}else{
      ...
      foreach($pdo->query("SELECT DISTINCT type FROM resource order by type") as $row){
			...
			echo "<tr><td>Login:</td><td><input type='text' value='".$filter."' name='type'></td>";
			...
			echo "<tr><td style='border-left: 1px solid green;' class='head'>ID</td><td class='head'>Name</td><td class='head'>Login</td><td class='head'>Company</td><td class='head'>Location</td><td class='head'>Category</td><td class='head'>Size</td><td class='head'>Cost</td><td class='head'>Auxdata</td></tr>";
			$i=0;
			...
			foreach($stmt as $key => $row){
					...
					echo "    <td class='result'>".(int)$row['size']."</td>\n";
					echo "    <td class='result'>".(int)$row['cost']."</td>\n";
					...
					echo "<input type='hidden' name='size' value='".(int)$row['size']."'>";
					echo "<input type='hidden' name='cost' value='".(int)$row['cost']."'>";
